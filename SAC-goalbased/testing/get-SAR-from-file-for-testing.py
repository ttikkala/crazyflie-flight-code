...
file_path_battery = '../training/' + folder_path_drone + 'Battery-20230706T16-15-09-timestamped.csv'
file_path_motors = '../training/' + folder_path_drone + 'Motors_fast-20230706T16-15-08-timestamped.csv'
file_path_stab = '../training/' + folder_path_drone + 'Stab-20230706T16-15-07-timestamped.csv'

folder_path_mocap = '../training/mocap_data/'
file_path_mocap = folder_path_mocap + 'Thu_Jul__6_16:15:17_2023/data.csv'
...
def clip_and_norm_actions(actions):
    ...
    means = pd.read_csv('../training/jul14-action_means_stds.csv')['Action means']
    stds = pd.read_csv('../training/jul14-action_means_stds.csv')['Action stds']
...
def normalise_state(state):
    ...
    means = pd.read_csv('../training/jul14-state_means_stds.csv')['State means']
    stds = pd.read_csv('../training/jul14-state_means_stds.csv')['State stds']

    for i in range(np.shape(state)[1]):
        state[:,i] = (state[:,i] - means[i]) / stds[i] 
...
def main():
    ...
    output_file = "./" + "actions-testing.csv"
    ...
    output_file = "./" + "states-testing.csv"
    ...
    state_means = pd.read_csv('../training/jul14-state_means_stds.csv')['State means']
    state_stds = pd.read_csv('../training/jul14-state_means_stds.csv')['State stds']
    ...
    output_file = "./" + "rewards-testing.csv"
