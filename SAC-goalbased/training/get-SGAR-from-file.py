...
def normalise_goal_pos(goal_pos, means, stds):
    ...
    for i in range(np.shape(goal_pos)[1]):
        goal_pos[i] = (goal_pos[i] - means[i]) / stds[i]
        goal_pos[i] = np.clip(goal_pos[i], -1, 1)
        goal_pos[i] *= 5
...
def calculate_rewards(state, goal_positions, stable_orientation):
    ...
    pos_error = np.array([state[:,0], state[:,1], state[:,2]]).transpose() - goal_positions
...
def choose_goal_position(means, stds, length):
    ...
    hover_range = range(0.3, 0.6, 0.05)

    goal_position_array = []
    goal_position = np.array([np.random.choice(hover_range), 0.0, 0.0])

    for i in range(length):
        if i % 100 == 0:
            goal_position = np.array([np.random.choice(hover_range), 0.0, 0.0])
        goal_position = normalise_goal_pos(goal_position, means, stds)
        goal_position_array.append(goal_position)

    return goal_position_array
...
def main():
    ...
    state_df = pd.DataFrame({'Pos y': state_data[:,0], 'Vel x': state_data[:,1], 'Vel z': state_data[:,2], 
                    ...
                    'goal pos y': goal_positions[0], 
                    'goal vel x': goal_positions[1], 
                    'goal vel z': goal_positions[2],
                    'goal quat w': np.ones(np.shape(state_data)[0]) * stable_orientation[0], 
                    'goal quat x': np.ones(np.shape(state_data)[0]) * stable_orientation[1], 
                    'goal quat y': np.ones(np.shape(state_data)[0]) * stable_orientation[2], 
                    'goal quat z': np.ones(np.shape(state_data)[0]) * stable_orientation[3]})
